...
import { createUserWithEmailAndPassword, onAuthStateChanged, signInWithEmailAndPassword, updateCurrentUser, User, UserCredential } from 'firebase/auth';
import { FirebaseError } from 'firebase/app';
import { useNavigation } from '@react-navigation/native';
import errorMessage from '../constants/Errors';
import { FontAwesomeIcon } from '@fortawesome/react-native-fontawesome';
import { faFacebookSquare, faGoogle, faApple } from '@fortawesome/free-brands-svg-icons';
import { addDoc, collection, CollectionReference, deleteDoc, deleteField, doc, DocumentData, DocumentReference, getDocs, getFirestore, QuerySnapshot, updateDoc } from 'firebase/firestore';
...
const LoginScreen = () => {
    ...
    const [passwordVerificationRequired, setPasswordVerificationRequired] = useState<boolean>(false);
    ...
    const exampleGET = () => {
        getDocs(collection(db, "users")).then((docRef: QuerySnapshot<DocumentData>) => {
            docRef.forEach((doc) => {
    ...
    const examplePOST = (data: any = { first: 'example', last: 'POST' }) => {
        addDoc(collection(db, "users"), data).then((docRef) => {
    ...
    const exampleDELETEDoc = () => {
        deleteDoc(doc(db, 'users', 'yo')).then((res) => {
        ...
        if (isPasswordValid() && isPasswordVerified()) {
            createUserWithEmailAndPassword(auth, email, password)
                .then(async (userCredentials: UserCredential) => {
                    const { uid, displayName, phoneNumber, photoURL, email, emailVerified } = userCredentials.user;
                    console.log({ uid, displayName, phoneNumber, photoURL, email, emailVerified });
            ...
            {!user &&
                <>
                    ...
                    <View style={styles.inputContainer}>
                        ...
                        {
                            passwordVerificationRequired &&
                            <Text style={styles.loginText} onPress={() => resetToLogin()}> Login </Text>
                        }
                    </View>
                    { error && 
                    ...
                    <View style={styles.buttonContainer}>
                        {
                            !passwordVerificationRequired &&
                            <TouchableOpacity style={styles.button} onPress={handleLogin} disabled={passwordVerificationRequired}>
                    ...
                    <View style={styles.providerContainer}>
                        <TouchableOpacity >
                            <FontAwesomeIcon icon={faFacebookSquare} size={32} />
                        </TouchableOpacity>
                        <TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    orContainer: { flexDirection: 'row', alignItems: 'center', margin: 10, width: '80%' },
    orLines: { flex: 1, height: 1, backgroundColor: '#2f95dc', width: 100 },
    orText: { width: 50, textAlign: 'center' },
    providerContainer: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-evenly', width: '80%' }
})